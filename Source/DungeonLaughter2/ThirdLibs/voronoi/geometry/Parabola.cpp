...
Parabola::Parabola(const VPoint& focus, double baselineY) : _focus(focus), _baselineY(baselineY)
{
    _stretchFactor = (_focus.y()-_baselineY)/2;
    _vertex = VPoint(_focus.x(), _focus.y()-_stretchFactor);
}

const VPoint& Parabola::focus() const
{
    return _focus;
}
...
const VPoint& Parabola::vertex() const
{
    return _vertex;
}
...
VPoint Parabola::at(double x) const
{
    return VPoint(x, (*this)(x));
}
...
ParabolaIntersectionSolutionSet Parabola::intersection(const Parabola& parabola) const
{
    ...
    if (a1==a2) { // parabolas have same stretch factor
        x = (h1+h2)/2;
    } else if (a1==0) { // first parabola lies on baseline -> degenerates to VPoint
        x = h1;
        return ParabolaIntersectionSolutionSet(parabola.at(x));
    } else if (a2==0) { // second parabola lies on baseline -> degenerates to VPoint
        x = h2;
    } else { // two intersection VPoints
...
ParabolaIntersectionSolutionSet::ParabolaIntersectionSolutionSet(const VPoint& point) : _type(ONE_SOLUTION), _leftPoint(point), _rightPoint(point)
{
}

ParabolaIntersectionSolutionSet::ParabolaIntersectionSolutionSet(const VPoint& point1, const VPoint& point2) : _type(TWO_SOLUTIONS), _leftPoint(point1), _rightPoint(point2)
{
...
const VPoint& ParabolaIntersectionSolutionSet::point() const
{
    return _leftPoint;
}

const VPoint& ParabolaIntersectionSolutionSet::leftPoint() const
{
    return _leftPoint;
}

const VPoint& ParabolaIntersectionSolutionSet::rightPoint() const
{
    return _rightPoint;
}
