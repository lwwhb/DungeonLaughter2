...
ConvexPolygon::ConvexPolygon(const std::vector<VPoint>& points)
{
    for (std::vector<VPoint>::const_iterator it = points.begin(); it != points.end(); ++it) {
        *this << *it;
    }
}

const std::vector<VPoint>& ConvexPolygon::points() const
{
    return _points;
}
...
bool ConvexPolygon::contains(const VPoint& p) const
{
    if (_edges.size() < 3) {
        return false;
    }
    ...
    for (unsigned i=0; i < _edges.size(); ++i) {
        const Line& line = _edges[i]; // edge from VPoints[i] to VPoints[i+1]
...
PolygonIntersectionSolutionSet ConvexPolygon::intersection(const Line& line) const
{
    ...
    for (std::vector<Line>::const_iterator it = _edges.begin(); it != _edges.end(); ++it) {
        ...
        if (solutionSet.isOne()) {
            result.addVPoint(solutionSet.point());
        }
...
ConvexPolygon& ConvexPolygon::operator<<(const VPoint& point)
{
    if (violatesConvexity(point)) {
        throw std::runtime_error("VPoint violates convexity of polygon");
    }
...
bool ConvexPolygon::violatesConvexity(const VPoint& point) const
{
    if (_points.size() < 3) {
        return false;
    }
...
std::vector<VPoint> PolygonIntersectionSolutionSet::points() const
{
    return std::vector<VPoint>(_points.begin(), _points.end());
}
...
void PolygonIntersectionSolutionSet::addVPoint(const VPoint& point)
{
